...
func (h handler) AddSong(c *gin.Context) { // Хэндлер добавления песни к себе
	...
	if err := c.ShouldBindJSON(&requestBody); err != nil { // Получение нужной песни из хапроса
	...
	h.DB.Raw("SELECT id FROM songs WHERE name = ?", strings.ToLower(requestBody.Song)).Scan(&songID) // Ищем в таблице песен айди песни по названию, сканим в переменную
